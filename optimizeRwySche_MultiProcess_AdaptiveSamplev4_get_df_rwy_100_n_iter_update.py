...
from SimSA_v4.TraciSim_highestODBased_get_df_rwy import highestODSim
import random
...
class optimizeRwySch(object):
    def __init__(self, airport, traffic_scenario, avg_run, n_iter_update):
        ...
        self.if_rwy_seq = config_sim['TrafficRule']['if_rwy_sequence']

        self.simulation_setting = str(config_sim['VariationSettings']['speedControl']) + '_' + str(config_sim['VariationSettings']['arrivalControl'] +'_'+str(config_sim['VariationSettings']['getPushControl'] + '_'+str(config_sim['TrafficRule']['if_rwy_sequence'])))
        self.simCacheLoc = self.optimizeFileLoc + 'sim_cache'+str(n_iter_update)+'/'

        createFolder(self.simCacheLoc)
    ...
    def objective_function_iter(self, dict_state, num_iteration, annotate):
        total_obj = np.zeros(3, dtype=np.float64)
        for avg_i in range(self.avg_run):
            number_of_conflicts, total_fuel_consumption, neg_airport_hourly_throughput, df_speed = self.traciSim.run_get_objective_iter(
                dict_state)
            df_speed.to_csv(self.simCacheLoc+'df_speed_' + str(num_iteration)+'_'+ str(self.n_iter_update)+'_'+ annotate + '_' +str(avg_i) + '.csv')
            # Add current run's results to the total
            total_obj = total_obj + np.array(
                [number_of_conflicts, total_fuel_consumption, neg_airport_hourly_throughput],
                dtype=np.float64)
    ...
    def neighbour(self, dict_state):
        ...
        while continue_sample == True:
            ...
            lst_index = random.sample(range(0, self.n_planes), self.n_update)
            for index in lst_index:
                new_state[index] = self.dict_rwy_id[random.randint(1, self.n_rwys)]
            dict_state_new = dict(zip(dict_state.keys(), new_state))
            if dict_state_new in self.lst_last_dict_state:
                ...
                continue_sample = False
                self.lst_last_dict_state.append(dict_state_new)
                return dict_state_new
    ...
    def find_optimal_objective_state(self, lst_new_energy, lst_dict_state_new, initial_E):
        ...
        print('lst_improve_energy:{}'.format(lst_improve_energy))
        ...
        if all(new_energy< self.best_E):
            self.best_E = new_energy
            self.best_dict_state = dict_state_new
        elif new_energy[0]< self.num_conflict_control and all(new_energy[1:] < self.best_E[1:]):
            self.best_E = new_energy
            self.best_dict_state = dict_state_new

        return dict_state_new, new_energy
    def fcfs_rwy(self, df):
        runway_names = list(self.dict_rwy_id.values())
        dict_min_rwy_sep = self.dict_min_rwy_sep
        df.loc[:, 'depart_time'] = df['start_time_in_minute_update']
        n_runway=len(runway_names)

        df = df.sort_values(by='depart_time').reset_index(drop=True)

        # Assign the first three aircraft directly to the runways
        id_initial_rwy = int(n_runway-1)
        df.loc[:id_initial_rwy, 'assigned_runway'] = runway_names[:n_runway]

        # Initialize runway tracking structures
        runway_last_times = {rwy: 0 for rwy in runway_names}  # Tracks last landing time on each runway
        runway_last_type = {rwy: None for rwy in runway_names}  # Tracks last aircraft type on each runway

        # Function to assign a runway while maintaining separation constraints
        # Function to assign a runway based on minimal sum of last_time + min_sep
        def assign_runway(depart_time, aircraft_type, runway_last_times, runway_last_type):
            best_runway = None
            min_time = float('inf')

            for rwy in runway_names:
                last_time = runway_last_times[rwy]
                last_type = runway_last_type[rwy]

                # Determine the required separation
                min_sep = dict_min_rwy_sep.get((last_type, aircraft_type), 30) if last_type else 0

                # Compute the effective available time
                effective_time = last_time + min_sep

                # Choose the runway with the minimal effective available time
                if effective_time < min_time:
                    min_time = effective_time
                    best_runway = rwy

            # Update the selected runway's last time and type
            runway_last_times[best_runway] = max(depart_time, min_time)
            runway_last_type[best_runway] = aircraft_type
            return best_runway, runway_last_times, runway_last_type

        # Assign aircraft to runways
        assigned_runways = []
        for id_veh in range(id_initial_rwy, len(df)):

            rwy, runway_last_times, runway_last_type = assign_runway(df['depart_time'].iloc[id_veh], df['ICAO_type'].iloc[id_veh],
                                                                     runway_last_times, runway_last_type)
            assigned_runways.append(rwy)


        df.loc[id_initial_rwy:,'assigned_runway'] = assigned_runways

        dict_state = dict(zip(list(df['veh_id'].values), list(df['assigned_runway'].values)))
        return dict_state

    def check_percentage_difference(self, arr1, arr2, threshold):
        if arr1.shape != arr2.shape:
            raise ValueError("Arrays must have the same shape")

        percentage_diff = (arr1 - arr2) / abs(arr1)

        if np.all(percentage_diff > threshold):
            return False
        else:
            return True
    ...
    def optimizeRwySche(self):
        ...
        expected_improved_E = np.array([0.1, 0.12, 0.12], dtype=np.float64)

        np_result_store = np.zeros([1, int((self.n_core+2) * self.objective_weight.shape[0] + 1)])

        with open(self.optimizeFileLoc + self.simulation_setting+'_'+ str(self.n_iter_update)+'.txt', 'w') as f:
            ...
            dict_state = self.fcfs_rwy(self.df_traffic_scenario)

            # Simulated annealing algorithm
            T = initial_T
            iteration = 0
            last_E = self.objective_function_iter(dict_state, self.avg_run, 'initial')
            ...
            self.n_update = self.n_update+self.n_iter_update
            ...
            while all(T > T_threshold) and iteration < max_iterations and continue_improve:
                ...
                continue_improve = self.check_percentage_difference(self.initial_E, self.best_E, expected_improved_E)
        ...
        best_E = self.objective_function_iter(self.best_dict_state, self.avg_run, 'best')
...
if __name__ == '__main__':
    lst_n_update = [6,5,4,3,2]
    for i in lst_n_update:
        print('This is '+str(i))
        now = datetime.datetime.now()
        print("Current date and time : {}".format(now.strftime("%Y-%m-%d %H:%M:%S")))
        _optimizeRwySch = optimizeRwySch(airport='KLAX_test', traffic_scenario='equal', avg_run=100, n_iter_update = i)
