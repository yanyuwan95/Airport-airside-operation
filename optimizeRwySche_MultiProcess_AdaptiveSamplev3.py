...
from SimSA_v2.TraciSim_highestODBased import highestODSim
import random
import math
...
class optimizeRwySch(object):
    ...
    def objective_function(self, dict_state):
        number_of_conflicts, total_fuel_consumption, neg_airport_hourly_throughput = self.traciSim.run_get_objective(
            dict_state)
        return np.array([number_of_conflicts, total_fuel_consumption, neg_airport_hourly_throughput],dtype=np.float64)
