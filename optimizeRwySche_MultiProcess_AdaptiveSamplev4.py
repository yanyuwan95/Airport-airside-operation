...
from SimSA_v3.TraciSim_highestODBased import highestODSim
import random
...
class optimizeRwySch(object):
    ...
    def objective_function(self, dict_state):
        ...
        for avg_i in range(self.avg_run):
            ...
            total_obj = total_obj + np.array([number_of_conflicts, total_fuel_consumption, neg_airport_hourly_throughput],
                                  dtype=np.float64)
        avg_obj = total_obj / self.avg_run
        return avg_obj

        #return np.array([avg_number_of_conflicts, avg_total_fuel_consumption, avg_neg_airport_hourly_throughput],dtype=np.float64)
    ...
    def optimizeRwySche(self):
        ...
        with open(self.optimizeFileLoc + 'optimize_constant_testTime.txt', 'w') as f:
            ...
            T = initial_T
            iteration = 0
            last_E = self.objective_function(dict_state)
        ...
        df_result.to_csv(self.optimizeFileLoc+'result_constant_testTime.csv', index=True, index_label='iteration')

if __name__ == '__main__':
    now = datetime.datetime.now()
    print("Current date and time : {}".format(now.strftime("%Y-%m-%d %H:%M:%S")))
    _optimizeRwySch = optimizeRwySch(airport='KLAX_test', traffic_scenario=1, avg_run = 2)
