...
from SimSA_v2.TraciSim_highestODBased_get_df_no_taxiway import highestODSim
import random
...
class optimizeRwySch(object):
    def __init__(self, airport, traffic_scenario):
        ...
        self.control_object_weight = np.array([0, 0, 0, 1], dtype=np.float64)

        self.changed_state = True
        self.lst_last_dict_state = []

        self.n_core = 5
        self.objective_weight = np.array([0, 0, 0, 1],dtype=np.float64)  # number_of_conflicts, total_fuel_consumption, total_taxi_time_for_all_air
    ...
    def add_initial_rwy(self, df):
        dict_assign = {'06R/24L':[0,1,2,3,9,11], '07L/25R':[4,5,6,7,8,12,13,14]}
    ...
    def objective_function(self, dict_state):
        number_of_conflicts, total_fuel_consumption, neg_airport_hourly_throughput, wait_time = self.traciSim.run_get_objective(
            dict_state)


        return np.array([number_of_conflicts, total_fuel_consumption, neg_airport_hourly_throughput, wait_time],dtype=np.float64)

    def objective_function_iter(self, dict_state, num_iteration):
        number_of_conflicts, total_fuel_consumption, neg_airport_hourly_throughput,  wait_time, df_speed= self.traciSim.run_get_objective_iter(
            dict_state)

        df_speed.to_csv('../../Data/KLAX_test/optimization/df_speed/' + str(num_iteration) + '.csv')
    ...
    def find_optimal_objective_state(self, lst_new_energy, lst_dict_state_new, initial_E):
        ...
        lst_improve_energy = [np.divide(x - initial_E, np.multiply(np.array([1, 1, -1, 1]), initial_E)) for x in lst_new_energy]
    ...
    def optimizeRwySche(self):
        ...
        initial_T = np.array([100, 100, 100, 100],dtype=np.float64)  # initial temperature
        cooling_factor = 0.99  # cooling factor
        T_threshold = np.array([0.1, 0.1, 0.1, 0.1],dtype=np.float64)  # temperature threshold

        np_result_store = np.zeros([1, int((self.n_core+2) * self.objective_weight.shape[0] + 1)])

        with open(self.optimizeFileLoc + 'optimize_constant_testTime.txt', 'w') as f:
        ...
        lst_objective = ['number_of_conflicts', 'total_fuel_consumption', 'neg_airport_hourly_throughput', 'runway out of usage time']
        ...
        df_result.to_csv(self.optimizeFileLoc+'no_taxiwayArea.csv', index=True, index_label='iteration')

if __name__ == '__main__':
    now = datetime.datetime.now()
    print("Current date and time : {}".format(now.strftime("%Y-%m-%d %H:%M:%S")))
    _optimizeRwySch = optimizeRwySch(airport='KLAX_test', traffic_scenario=1)
